...
int dScan(char* targetPath, char* info) {
	...
	while (isThereDir) {
		...
		while ((contentDir = readdir(dTarget)) != NULL) {
			...
			char* fullPath = (char*) malloc(newSize * sizeof(char));
			...
			subPath = (char**)realloc(subPath, cSize * sizeof(char*));
			subPath[cSize] = (char*)malloc(strlen(fullPath) + 1);
